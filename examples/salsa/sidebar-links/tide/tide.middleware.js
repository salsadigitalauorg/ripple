export default {
  sideBarLinks: async (context, pageData) => {
    if (pageData.tidePage.field_sidebar_links) {
      const sideBarLinks = pageData.tidePage.field_sidebar_links

      if (sideBarLinks && sideBarLinks.length > 0) {
        for (const linkBlock of sideBarLinks) {
          if (linkBlock && linkBlock.field_paragraph_reference_links && linkBlock.field_paragraph_reference_links.length > 0) {
            const links = linkBlock.field_paragraph_reference_links
